...
static std::optional<std::string> read_file(const std::string &file) {
  ...
  try {
    return std::make_optional(std::string(std::istreambuf_iterator<char>(is),
...
void File::read() const {
  if (!text) // could do that to avoid re-reading, but force re-reading does not
             // take too long so do it just to be sure
    text = read_file(path);
  if (!text)
...
std::optional<std::shared_ptr<Project>>
Workspace::project_for_file(const std::filesystem::path &file) const {
  // try active project
  if (active_project.has_value()) {
    auto active =
        std::find_if(projects_.all_projects.begin(),
                     projects_.all_projects.end(), [&](const auto &project) {
  ...
  for (const auto &project : projects_.all_projects) {
    for (const auto &f : project->files) {
      // do not use recursive check, as dependencies are also contained in
      // all_projects
      if (f->path == file) {
        return project;
      }
...
void Workspace::mark_file_dirty(const std::filesystem::path &file) {
  if (auto it = projects_.tracked_files.find(file);
      it != projects_.tracked_files.end()) {
    it->second->text = std::nullopt;
  }
}
void Workspace::set_file_content(const std::filesystem::path &file,
                                 std::string &&content) {
  if (auto it = projects_.tracked_files.find(file);
      it != projects_.tracked_files.end()) {
