...
Server::FileType Server::get_file_type(const std::filesystem::path &file) {
  return file.extension() == ".json" ? FileType::ConfigFile
                                     : FileType::SourceFile;
}

lsp::Location convert_loc(const Loc &loc) {
  if (!loc.file)
    throw lsp::RequestError(lsp::Error::InternalError,
                            "Cannot convert location with undefined file");
  return lsp::Location{
      .uri = lsp::FileUri::fromPath(*loc.file),
      .range = lsp::Range{
          .start = lsp::Position{static_cast<lsp::uint>(loc.begin.row - 1),
                                 static_cast<lsp::uint>(loc.begin.col - 1)},
          .end = lsp::Position{static_cast<lsp::uint>(loc.end.row - 1),
                               static_cast<lsp::uint>(loc.end.col - 1)}}};
}

Loc convert_loc(const lsp::TextDocumentIdentifier &file,
                const lsp::Position &pos) {
  return Loc(std::make_shared<std::string>(file.uri.path()),
             Loc::Pos{.row = static_cast<int>(pos.line + 1),
                      .col = static_cast<int>(pos.character + 1)});
}

Loc convert_loc(const lsp::TextDocumentIdentifier &file,
                const lsp::Range &pos) {
  return Loc(std::make_shared<std::string>(file.uri.path()),
             Loc::Pos{.row = static_cast<int>(pos.start.line + 1),
                      .col = static_cast<int>(pos.start.character + 1)},
             Loc::Pos{.row = static_cast<int>(pos.end.line + 1),
                      .col = static_cast<int>(pos.end.character + 1)});
}
...
InitOptions parse_initialize_options(const reqst::Initialize::Params &params,
  ...
  if (params.rootUri.isNull())
    throw lsp::RequestError(lsp::Error::InvalidParams,
                            "No root URI provided in initialize request");

  InitOptions data;
  data.workspace_root = std::string(params.rootUri.value().path());

  if (auto init = params.initializationOptions;
      init.has_value() && init->isObject()) {
    const auto &obj = init->object();
    if (auto it = obj.find("workspaceConfig");
        it != obj.end() && it->second.isString())
      data.workspace_config_path = it->second.string();

    if (auto it = obj.find("globalConfig");
        it != obj.end() && it->second.isString())
      data.global_config_path = it->second.string();

    if (auto it = obj.find("restartFromCrash");
        it != obj.end() && it->second.isBoolean())
      data.restart_from_crash = it->second.boolean();
  } else {
    server.send_message(
        "No initialization options provided in initialize request",
        lsp::MessageType::Error);
  }

  if (data.workspace_config_path.empty())
    server.send_message("No local artic.json workspace config",
                        lsp::MessageType::Warning);
  if (data.global_config_path.empty())
    server.send_message("No global artic.json config",
                        lsp::MessageType::Warning);
  return data;
}

void Server::setup_events_initialization() {
  message_handler_.add<reqst::Initialize>(
      [this](reqst::Initialize::Params &&params) -> reqst::Initialize::Result {
        Timer _("Initialize");
        log::info("\n[LSP] <<< Initialize");
        ...
        workspace_ = std::make_unique<workspace::Workspace>(
            init_data.workspace_root, init_data.workspace_config_path,
        ...
        return reqst::Initialize::Result{
            .capabilities =
                lsp::ServerCapabilities{
                    ...
                    .renameProvider =
                        lsp::RenameOptions{.prepareProvider = true},
                                ...
                                lsp::SemanticTokensLegend{
                                    ...
                                    .tokenModifiers =
                                        {"declaration", "definition",
                                         ...
                                         "abstract", "async", "modification",
                                         "documentation", "defaultLibrary"}},
                            .range = true,
                            .full = lsp::SemanticTokensOptionsFull{.delta =
                                                                       false}},
                    .inlayHintProvider =
                        lsp::InlayHintOptions{.resolveProvider = false}},
            .serverInfo = lsp::InitializeResultServerInfo{
                .name = "Artic Language Server", .version = "0.1.0"}};
      });

  message_handler_.add<notif::Initialized>(
      [this](notif::Initialized::Params &&) {
        log::info("\n[LSP] <<< Initialized");
        reload_workspace();
      });
...
void Server::setup_events_modifications() {
  ...
  message_handler_.add<notif::TextDocument_DidClose>(
      [](notif::TextDocument_DidClose::Params &&params) {
        log::info("\n[LSP] <<< TextDocument DidClose");
      });
  message_handler_.add<notif::TextDocument_DidOpen>(
      [this](notif::TextDocument_DidOpen::Params &&params) {
        log::info("\n[LSP] <<< TextDocument DidOpen");

        if (get_file_type(params.textDocument.uri.path()) ==
            FileType::SourceFile) {
          auto path = std::string(params.textDocument.uri.path());

          // skip compilation on open when it was already compiled
          // we need to do this as go to definition shortly opens the text
          // document in vscode and we don't want to invalidate the definition
          // while looking it up
  ...
  message_handler_.add<notif::TextDocument_DidChange>(
      [this](notif::TextDocument_DidChange::Params &&params) {
        log::info("");
        log::info("--------------------------------");
        ...
        auto &content = std::get<lsp::TextDocumentContentChangeEvent_Text>(
                            params.contentChanges[0])
                            .text;
        compile_file(file, &content);
      });

  message_handler_.add<notif::TextDocument_DidSave>(
      [this](notif::TextDocument_DidSave::Params &&params) {
  ...
  message_handler_.add<notif::Workspace_DidChangeConfiguration>(
      [this](notif::Workspace_DidChangeConfiguration::Params &&params) {
        log::info("\n[LSP] <<< Workspace DidChangeConfiguration");
        // Optionally, could inspect params.settings to override paths.
        reload_workspace();
      });
  message_handler_.add<notif::Workspace_DidChangeWatchedFiles>(
      [this](notif::Workspace_DidChangeWatchedFiles::Params &&params) {
        for (auto &change : params.changes) {
          ...
          case lsp::FileChangeType::Changed:
            break; // Handle elsewhere
          case lsp::FileChangeType::MAX_VALUE:
            break;
          }
        }
      });
...
SemanticToken create_semantic_token(const Loc &loc, const ast::NamedDecl &decl,
                                    bool is_decl) {
  SemanticToken token{
  ...
  if (auto t = decl.isa<ast::StaticDecl>()) {
    ...
    token.type = (uint32_t)ty::Type;
  else if (decl.isa<ast::FnDecl>())
    token.type = (uint32_t)ty::Function;
  else if (decl.isa<ast::RecordDecl>())
    token.type = (uint32_t)ty::Struct;
  else if (decl.isa<ast::EnumDecl>())
    token.type = (uint32_t)ty::Enum;
  else if (decl.isa<ast::TypeDecl>())
    token.type = (uint32_t)ty::Type;
  else if (decl.isa<ast::FieldDecl>())
    token.type = (uint32_t)ty::Property;
  else if (decl.isa<ast::ModDecl>())
    token.type = (uint32_t)ty::Namespace;
  else if (decl.isa<ast::UseDecl>())
    token.type = (uint32_t)ty::Namespace;
  ...
  auto type = decl.type;
  if (type) {
    if (auto addr = type->isa<AddrType>(); addr && addr->pointee)
      type = addr->pointee; // remove reference
    if (auto app = type->isa<TypeApp>(); app && app->applied)
      type = app->applied; // collapse polymorphic type
...
// Collect semantic tokens from the NameMap by iterating over declarations and
// references
lsp::SemanticTokens collect(const ls::NameMap &name_map,
                            const std::string &file, int start_row = 0,
                            int end_row = std::numeric_limits<int>::max()) {
  std::vector<SemanticToken> tokens;
  // Check if we have entries for this file
  if (!name_map.files.contains(file))
    return {};

  auto &names = name_map.files.at(file);

  // Collect tokens from references (this is where we want semantic
  // highlighting)
  ...
  for (const auto &token : tokens) {
    ...
    uint32_t delta_start =
        (delta_line == 0) ? token.start - prev_start : token.start;
...
void Server::setup_events_tokens() {
  // Semantic Tokens
  // ----------------------------------------------------------------------
  message_handler_.add<reqst::TextDocument_SemanticTokens_Full>(
      [this](lsp::SemanticTokensParams &&params)
          -> reqst::TextDocument_SemanticTokens_Full::Result {
        ...
        // semantic tokens are not allowed to trigger recompile as this is
        // called right after document changed
        bool already_compiled = compile && compile->locator.data(file);
        if (!already_compiled)
          return nullptr;
        // ensure_compile(file);
        auto tokens = collect(compile->name_map,
  ...
  message_handler_.add<reqst::TextDocument_SemanticTokens_Range>(
      [this](lsp::SemanticTokensRangeParams &&params)
          -> reqst::TextDocument_SemanticTokens_Range::Result {
        Timer _("TextDocument_SemanticTokens_Range");
        std::string file(params.textDocument.uri.path());
        log::info(
            "\n[LSP] <<< TextDocument SemanticTokens_Range {}:{}:{} to {}:{}",
            file, params.range.start.line + 1, params.range.start.character + 1,
...
std::optional<IndentifierOccurences>
find_occurrences_of_identifier(Server &server, const Loc &cursor,
...
void Server::setup_events_definitions() {
  message_handler_.add<reqst::TextDocument_Definition>(
      [this](lsp::TextDocumentPositionParams &&pos)
          -> reqst::TextDocument_Definition::Result {
        Timer _("TextDocument_Definition");
        log::info("\n[LSP] <<< TextDocument Definition {}:{}:{}",
                  pos.textDocument.uri.path(), pos.position.line + 1,
        ...
        if (auto ref = name_map.find_ref_at(cursor)) {
          if (auto def = name_map.find_decl(*ref)) {
            ...
            log::info("[LSP] >>> return TextDocument Definition {}:{}:{}",
                      loc.uri.path(), loc.range.start.line + 1,
        ...
        if (auto occurences =
                find_occurrences_of_identifier(*this, cursor, false)) {
          log::info("[LSP] >>> Found {} occurrences of identifier",
                    occurences->all_occurences.size());
          if (occurences->all_occurences.empty())
            return {occurences->declaration_range};
  ...
  message_handler_.add<reqst::TextDocument_References>(
      [this](lsp::ReferenceParams &&params)
          -> reqst::TextDocument_References::Result {
        Timer _("TextDocument_References");
        log::info("\n[LSP] <<< TextDocument References {}:{}:{}",
                  params.textDocument.uri.path(), params.position.line + 1,
  ...
  message_handler_.add<reqst::TextDocument_PrepareRename>(
      [this](lsp::TextDocumentPositionParams &&params)
          -> reqst::TextDocument_PrepareRename::Result {
        ...
        log::info("[LSP] >>> PrepareRename successful for symbol '{}'",
                  occurences->name);
  ...
  message_handler_.add<reqst::TextDocument_Rename>(
      [this](lsp::RenameParams &&params) -> reqst::TextDocument_Rename::Result {
        Timer _("TextDocument_Rename");
        log::info("\n[LSP] <<< TextDocument Rename {}:{}:{} -> '{}'",
                  params.textDocument.uri.path(), params.position.line + 1,
                  params.position.character + 1, params.newName);
        ...
        for (auto &loc : occurences->all_occurences) {
          changes[loc.uri].emplace_back(
              lsp::TextEdit{.range = loc.range, .newText = params.newName});
          ++total_edits;
        }

        log::info(
            "[LSP] >>> Rename operation will edit {} files with {} total edits",
            workspace_edit.changes->size(), total_edits);
...
lsp::CompletionItemKind get_completion_kind(const ast::NamedDecl *decl) {
  ...
  if (decl->isa<ast::PtrnDecl>())
    return lsp::CompletionItemKind::Variable;
  if (decl->isa<ast::StructDecl>())
    return lsp::CompletionItemKind::Struct;
  if (decl->isa<ast::EnumDecl>())
    return lsp::CompletionItemKind::Enum;
  if (decl->isa<ast::TypeDecl>())
    return lsp::CompletionItemKind::TypeParameter;
  if (decl->isa<ast::FieldDecl>())
    return lsp::CompletionItemKind::Field;
  if (decl->isa<ast::ModDecl>())
    return lsp::CompletionItemKind::Module;
  return lsp::CompletionItemKind::Text;
}

bool same_file(const Loc &a, const Loc &b) {
  return a.file && b.file && *a.file == *b.file;
}
bool overlaps(const Loc &a, const Loc &b) {
  return a.end > /* important > */ b.begin && a.begin <= b.end;
}
...
lsp::CompletionItem completion_item(const ast::FnDecl *fn) {
  ...
  if (const auto *type = fn->type) {
    if (const auto *forall = fn->type->isa<ForallType>())
      type = forall->body;
    if (type)
      if (const auto *f = fn->type->isa<FnType>()) {
        f->codom->print(l);
        item.detail = lb.str();
      }
  }
...
std::optional<lsp::CompletionItem> completion_item(const ast::NamedDecl &decl) {
  if (decl.id.name.empty())
    return std::nullopt;
  if (decl.id.name.starts_with('_'))
    return std::nullopt;

  if (auto fn = decl.isa<ast::FnDecl>())
    return completion_item(fn);
...
void Server::setup_events_completion() {
  message_handler_.add<reqst::TextDocument_Completion>(
      [this](lsp::CompletionParams &&params)
          -> reqst::TextDocument_Completion::Result {
        ...
        auto is_type_decl = [](const ast::NamedDecl &decl) -> bool {
          return decl.isa<ast::CtorDecl>() || decl.isa<ast::ModDecl>() ||
                 decl.isa<ast::TypeParam>() || decl.isa<ast::TypeDecl>() ||
                 decl.isa<ast::UseDecl>();
        };
        ...
        ast::Node::TraverseFn traverse([&](const ast::Node &node) -> bool {
          if (!node.loc.file)
            return true; // super module
          if (!same_file(cursor, node.loc))
            return false;
          if constexpr (debug_print)
            log::info("test node at {} vs {}", node.loc, cursor);
          ...
          if (!only_show_types &&
              (node.isa<ast::TypedExpr>() || node.isa<ast::TypedPtrn>() ||
               node.isa<ast::TypeApp>())) {
            only_show_types = true;
          } else if (const auto *mod = node.isa<ast::ModDecl>()) {
            current_module = mod;
          } else if (const auto *fn = node.isa<ast::FnDecl>()) {
            if (fn->fn->param)
              local_scopes.push_back(fn->fn->param.get());
            if (fn->type_params)
              local_scopes.push_back(fn->type_params.get());
          } else if (const auto *block = node.isa<ast::BlockExpr>()) {
            local_scopes.push_back(block);
            inside_block_expr = true;
            top_level = false;
          } else if (const auto *error = node.isa<ast::ErrorDecl>();
                     error && error->is_top_level) {
            top_level = true;
          }
          inner_node = &node;
        ...
        // ---
        // One possible modifier `only_show_types` if inside typed expression `a
        // : type`
        //
        // Different completion contexts:
        // 1. Projection expression `a.b`
        // 2. Path expression `a::b` (do not count if its just a single
        // identifier `a`) | uses `only_show_types`
        // 3. Top level declarations `struct a`
        //
        // 4. Default: (includes case where inner_node cannot be identified) |
        // uses `only_show_types` Show top_level decls in current module If
        // inside block expr, also show local declarations
        ...
        if (inner_node) {
          ...
          if (const auto *proj_expr = inner_node->isa<ast::ProjExpr>()) {
            ...
            if (auto t = proj_expr->type; t && !t->isa<TypeError>())
              type = t;
            else if (auto t = proj_expr->expr->type; t && !t->isa<TypeError>())
              type = t;
            if (type) {
              if (auto addr = type->isa<AddrType>(); addr && addr->pointee)
                type = addr->pointee; // remove reference
              ...
              if (auto struct_type = type->isa<StructType>()) {
                for (auto &field : struct_type->decl.fields) {
                  if (auto item = completion_item(*field))
          ...
          if (const auto *path = inner_node->isa<ast::Path>();
              path && path->elems.size() > 1) {
        ...
        if (inside_block_expr) {
          ...
          ast::Node::TraverseFn collect_local_decls([&](const ast::Node &node)
                                                        -> bool {
            // TODO this shows for definitions outside the loop `for a in ...`
            // -> shows `a`
...
void Server::compile_file(const std::filesystem::path &file,
  ...
  if (auto proj = workspace_->project_for_file(file)) {
    ...
    log::info(
        "Compiling file {} (not in workspace -> using default project {})",
        file, default_proj->name);
  ...
  auto convert_diagnostic = [](const Diagnostic &diag) -> lsp::Diagnostic {
    ...
    lsp_diag.range = lsp::Range{
        .start = lsp::Position{static_cast<uint>(diag.loc.begin.row - 1),
                               static_cast<uint>(diag.loc.begin.col - 1)},
        .end = lsp::Position{static_cast<uint>(diag.loc.end.row - 1),
                             static_cast<uint>(diag.loc.end.col - 1)}};
  ...
  for (const auto *file : files) {
    ...
    message_handler_.sendNotification<notif::TextDocument_PublishDiagnostics>(
        notif::TextDocument_PublishDiagnostics::Params{
            ...
            .diagnostics = diagnostics_by_file.contains(path)
                               ? diagnostics_by_file.at(path)
...
void Server::ensure_compile(std::string_view file_view) {
  ...
  if (get_file_type(file_view) != FileType::SourceFile) {
    throw lsp::RequestError(lsp::Error::InvalidParams,
                            "File is not an Artic source file");
  }
  bool already_compiled = compile && compile->locator.data(file);
  if (!already_compiled)
    compile_file(file);
  if (!compile)
    throw lsp::RequestError(lsp::Error::InternalError,
                            "Did not get a compilation result");
}
...
using FileDiags =
    std::unordered_map<std::filesystem::path, std::vector<lsp::Diagnostic>>;

void make_config_diagnostic(
    const workspace::config::ConfigLog::Message &msg,
    std::optional<std::filesystem::path> propagate_to_file, FileDiags &diags) {
  auto find_in_file = [](std::filesystem::path const &file,
                         std::string_view literal) -> std::vector<lsp::Range> {
    ...
    while (std::getline(ifs, line)) {
      ...
      while (pos != std::string::npos) {
        ranges.push_back(lsp::Range{
            ...
            lsp::Position{line_number,
                          static_cast<lsp::uint>(pos + literal.size())}});
  ...
  if (propagate_to_file) {
    if (propagate_to_file.value() == msg.file)
      return;
...
void Server::reload_workspace(const std::string &active_file) {
  ...
  if (print_to_console) {
    ...
    for (auto &e : log.messages) {
      if (e.severity > lsp::DiagnosticSeverity::Warning)
        continue;
      auto s = (e.severity == lsp::DiagnosticSeverity::Error)     ? "Error"
...
void Server::setup_events_other() {
  ...
  message_handler_.add<artic::reqst::DebugAst>(
      [this](lsp::TextDocumentPositionParams &&params)
          -> artic::reqst::DebugAst::Result {
        ...
        ast::Node::TraverseFn traverse([&](const ast::Node &node) -> bool {
          if (node.loc.file && same_file(node.loc, cursor) &&
              overlaps(node.loc, cursor)) {
            if (!outer_node) {
              outer_node = &node;
            }
            inner_node = &node;
  ...
  message_handler_.add<reqst::TextDocument_InlayHint>(
      [this](reqst::TextDocument_InlayHint::Params &&params)
          -> reqst::TextDocument_InlayHint::Result {
        Timer _("TextDocument_InlayHint");
        std::string file(params.textDocument.uri.path());
        log::info("\n[LSP] <<< TextDocument InlayHint {}:{}:{} to {}:{}", file,
                  params.range.start.line + 1, params.range.start.character + 1,
                  params.range.end.line + 1, params.range.end.character + 1);

        // inlay hints are not allowed to trigger recompile as this is called
        // right after document changed
        bool already_compiled = compile && compile->locator.data(file);
        if (!already_compiled)
          return nullptr;
        // ensure_compile(file);
        ...
        for (const auto *hint :
             compile->name_map.files.at(file).with_type_hint) {
          auto &loc = hint->loc;
          auto *type = hint->type;
          ...
          lsp::Position hint_pos{static_cast<lsp::uint>(loc.end.row - 1),
          ...
          if (hint_pos.line < params.range.start.line ||
              ...
              (hint_pos.line == params.range.end.line &&
               hint_pos.character > params.range.end.character)) {
            continue;
          }
