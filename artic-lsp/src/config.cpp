...
static inline void
collect_projects_recursive(const config::ConfigDocument &config,
                           CollectProjectsData &data, int depth = 0) {
  log::info("Collecting projects from config: {}", config.path.string());
  if (data.visited_configs.contains(config.path))
    return;
  ...
  for (const auto &proj : config.projects) {
    if (data.projects.contains(proj.name)) {
      ...
      log.warn("ignoring duplicate definition of " + proj.name + " in " +
                   proj.origin.string(),
               proj.name);
      continue;
    }
  ...
  for (const auto &include : config.includes) {
    ...
    if (auto include_config = config::ConfigDocument::parse(include, log)) {
      ...
      log.info("Path: \"" + include.path.string() + "\"",
               include.raw_path_string);
    }
  }
}
...
find_matching_glob(std::filesystem::path root, std::string glob_pattern,
                   config::ConfigLog &log) {
  ...
  if (glob_pattern[0] == '~') {
    ...
    if (home) {
      ...
      log.warn("Cannot expand ~ in pattern: " + glob_pattern +
                   " $HOME is undefined",
               glob_pattern);
      return results;
    }
  ...
  auto is_wild = [](const std::string &s) {
  ...
  std::function<void(size_t, const std::filesystem::path &)> dfs =
      [&](size_t idx, const std::filesystem::path &base) {
        if (idx == parts.size()) {
          ...
          if (std::filesystem::is_regular_file(base)) {
            ...
            if (dedup.insert(norm).second)
              results.emplace_back(norm);
          }
          return;
        }
        ...
        // Special case: '**' as its own segment matches zero or more directory
        // levels.
        if (part == "**") {
          ...
          for (auto it = std::filesystem::directory_iterator(base);
            ...
            if (++dir_count > 50'000) { // arbitrary safety cap
              log.warn("Stopped expanding '**' due to excessive directories",
                       part);
              break;
            }
        ...
        // If last component and refers to a file name directly without
        // wildcards
        if (!is_wild(part)) {
          ...
          if (idx + 1 == parts.size()) {
            ...
            return; // even if it is directory but pattern ended, we only
                    // collect files
          } else {
        ...
        // Wildcard segment (but not **) -> enumerate entries in this directory
        // only
        size_t checked = 0;
        for (auto it = std::filesystem::directory_iterator(base);
             it != std::filesystem::directory_iterator(); ++it) {
          if (++checked > 100'000) {
            log.warn("Stopped expanding wildcard: too many entries", part);
            break;
          }
...
static std::shared_ptr<Project> instantiate_project(
    ...
    std::unordered_map<std::filesystem::path, std::shared_ptr<File>>
        &tracked_files,
    config::ConfigLog &log) {
  ...
  // Collect all files matching include patterns and not matching exclude
  // patterns
  std::unordered_set<std::filesystem::path> matched_files;

  auto file_arr_to_string = [](const std::filesystem::path &root_dir,
                               const auto &files) {
    std::ostringstream s;
    s << files.size() << " files:" << std::endl;
    for (const auto &file : files) {
      s << "- " << std::filesystem::relative(file, root_dir).string() << " "
        << std::endl;
    }
    return s.str();
  };
  ...
  for (const auto &pattern : include_patterns) {
    if (proj_def.was_defined_in_global_config &&
        pattern.find("**") != std::string::npos) {
      log.error("Recursive include patterns are not allowed in global config",
                pattern);
      continue;
    }
    ...
    log.info("+ " + std::to_string(after - before) + " files" +
  ...
  for (const auto &pattern : exclude_patterns) {
    ...
    log.info("- " + std::to_string(before - after) + " files" +
                 " | total matches: " + file_arr_to_string(root_dir, matches),
             pattern);
  }
...
void Workspace::reload(config::ConfigLog &log) {
  ...
  config::IncludeConfig include_global{.path = global_config_path,
                                       .raw_path_string = "<global>",
                                       .is_optional = false,
                                       .is_global = true};
  config::IncludeConfig include_local{.path = workspace_config_path,
                                      .raw_path_string = workspace_config_path,
                                      .is_optional = false,
                                      .is_global = false};
  ...
  if (global_config_path.empty()) {
    log.warn("Missing global config: no path specified in settings",
             "<global>");
  } else {
    if (auto global_config =
            config::ConfigDocument::parse(include_global, log)) {
  ...
  } else if (auto local_config =
                 config::ConfigDocument::parse(include_local, log)) {
    collect_projects_recursive(local_config.value(), data);
    if (auto &dp = local_config->default_project) {
      default_project = dp.value();
    }
    active_project =
        local_config->projects.empty()
            ? std::nullopt
            : std::make_optional(local_config->projects.front().name);
  }
  ...
  for (const auto &[id, def] : project_defs) {
    projects[id] = {.project =
                        instantiate_project(def, projects_.tracked_files, log),
  ...
  auto log_project_info = [&](const Project &dep,
                              const std::filesystem::path &current_config) {
  ...
  for (auto &[id, p] : projects) {
    ...
    for (auto &dep_id : p.dependencies) {
      if (projects.contains(dep_id)) {
        ...
        // log.error("Failed to resolve dependency " + dep_id + " for project "
  ...
  if (default_project) {
    auto project =
        instantiate_project(*default_project, projects_.tracked_files, log);
    ...
    for (auto &dep_id : default_project->dependencies) {
      if (projects.contains(dep_id)) {
        ...
        log.error("Failed to resolve dependency " + dep_id +
                      " for default project " + project->name,
                  dep_id);
      }
    }
...
to_absolute_path(const std::filesystem::path &base_dir, std::string_view path) {
  if (path.starts_with("~")) {
...
std::optional<ConfigDocument> ConfigDocument::parse(const IncludeConfig &config,
                                                    ConfigLog &log) {
  ...
  if (!std::filesystem::exists(config.path)) {
    if (!config.is_optional)
      log.error("Config file does not exist: \"" + config.path.string() + "\"",
                config.raw_path_string);
    return std::nullopt;
  }
  log.file_context = config.path;
  try {
    ...
    if (!j.contains("artic-config")) {
      log.error("Missing artic-config header\n"
                "Example: \"artic-config\": \"1.0\"");
      return std::nullopt;
    }
    for (auto &[key, value] : j.items()) {
      if (key == "artic-config" || key == "default-project" ||
          key == "include" || key == "projects")
        continue;
    ...
    if (doc.version != "1.0") {
      log.warn("Unsupported artic-config version (Should be 1.0)",
               "artic-config");
    }

    auto parse_project =
        [&](const nlohmann::json &pj) -> std::optional<ProjectDefinition> {
      ProjectDefinition p;
      if (!pj.contains("name")) {
        log.error("Every project must have a name"
                  "\nExample: " +
                      nlohmann::json{{"name", "my_project"}}.dump(),
                  "projects");
        return std::nullopt;
      }
      ...
      if (folder_ptrn.empty()) {
        ...
        if (std::filesystem::exists(res) &&
            std::filesystem::is_directory(res)) {
          p.root_dir = std::filesystem::weakly_canonical(res);
        } else {
          log.error("Project folder does not exist: " + res.string(),
                    pj.value<std::string>("folder", ""));
          p.root_dir = root;
        }
      }
    ...
    if (j.contains("include")) {
      ...
      if (config.is_global && include_global) {
        log.warn(
            "including <global> in the global configuration file has no effect",
            "<global>");
      }
      if (!config.is_global && !include_global) {
        log.warn("add '<global>' to improve readability as global projects are "
                 "implicitly included",
                 "include");
      }
    }
    ...
    log.error(std::string("Failed to parse json ") + config.path.string() +
              ": " + e.what());
    return std::nullopt;
  }
}
