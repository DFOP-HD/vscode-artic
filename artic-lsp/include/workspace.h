...
struct Project {
    ...
    struct PatternMatch {
        ...
        size_t changed = 0;
        bool excludes = false;
        // Where the pattern was written in `origin`, so the hint lands on the right one when
        // two projects share a pattern. Unset for a project taken from a build file.
    ...
    // Where the name was written in `origin`. Unset for a project taken from a build file.
    std::optional<lsp::Range> name_range;
