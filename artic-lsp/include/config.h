...
struct ConfigDocument {
  ...
  static std::optional<ConfigDocument> parse(const IncludeConfig &config,
                                             ConfigLog &log);
};
...
struct ConfigLog {
  ...
  void error(std::string msg,
    ...
    messages.push_back(make_message(Severity::Error, std::move(msg), context));
  }
  void warn(std::string msg,
            std::optional<std::string> context = std::nullopt) {
    messages.push_back(
        make_message(Severity::Warning, std::move(msg), context));
  }
  void info(std::string msg,
            std::optional<std::string> context = std::nullopt) {
    messages.push_back(
        make_message(Severity::Information, std::move(msg), context));
  }
...
private:
  ...
  Message make_message(Severity s, std::string msg,
                       std::optional<std::string> context) {
    return Message{
        .message = std::move(msg),
        .severity = s,
        .file = file_context,
        .context = context ? std::make_optional(Context{quote(context.value())})
                           : std::nullopt};
  }
};
