...
| `zero-config.test.mjs` | A workspace with build files but no `artic.json`: the sources still compile together, a config the user wrote wins over a detected build file, the nearest build file wins over a farther one, an unrelated tree full of build files does not starve the scan, two checkouts of the same project each keep their own, and the scan does not descend into hidden or output directories. |
